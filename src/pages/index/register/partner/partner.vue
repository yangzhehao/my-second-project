<template>
    <view class="page">
        <view class="header">
            合伙人<text class="status red">（注册中）</text>
        </view>
        <view class="form">
            <view class="item">
                <div class="item_label">合伙人姓名</div>
                <input type="text" class="input" placeholder="请输入合伙人姓名">
            </view>
            <view class="item">
                <div class="item_label">联系电话</div>
                <input type="text" class="input" placeholder="请输入合伙人的手机号">
            </view>
...
const pickerChange = (currentString, e) => {
    if (currentString === 'address') {
        ...
        //如果是触发第2列的时候有一种情况,就是先触发第二列,再触发第一列,这时候需要判断第一列的数组索引对应的数据长度和第二列的索引的对等关系
        // if (diffIndex == 1) {
